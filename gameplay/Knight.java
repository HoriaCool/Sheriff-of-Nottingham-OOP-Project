// Copyright 2019 Nedelcu Horia (nedelcu.horia.alexandru@gmail.com)

package gameplay;

public final class Knight extends Player {
    private static final int BASE_HP = 900;
    private static final int LEVEL_HP = 80;

    private static final float LAND_BONUS = 1.15f;

    private static final int DMG_BASE1 = 200;
    private static final int DMG_LEVEL1 = 30;

    private static final float P1_PYRO = 1.1f;
    private static final float P1_KNIG = 1f;
    private static final float P1_WIZZ = 0.8f;
    private static final float P1_ROGU = 1.15f;

    private static final int DMG_BASE2 = 100;
    private static final int DMG_LEVEL2 = 40;
    private static final int PER_DMG_BASE = 0;
    private static final int PER_DMG_LEVEL = 0;

    private static final float P2_PYRO = 0.9f;
    private static final float P2_KNIG = 1.2f;
    private static final float P2_WIZZ = 1.05f;
    private static final float P2_ROGU = 0.8f;

    private static final float P_BASE_HP = 0.2f;
    private static final float P_LEVEL_HP = 0.01f;
    private static final float P_MAXLIM_HP = 0.4f;

    Knight(final int x, final int y, final String type) {
        super(LEVEL_HP, BASE_HP, x, y, type);
    }

    private void doKnightAbility(final Player player,
            final float procent1, final float procent2) {
        if (player.mHitPoints < Math.round(Math.min(P_BASE_HP + P_LEVEL_HP * mLevel, P_MAXLIM_HP)
                * player.getFullLife())) {
            player.mHitPoints = 0;
            return;
        }

        int damage = Math.round(procent1 * getDamage1());
        damage += Math.round(procent2 * getDamage2());
        player.mHitPoints = Math.max(0, player.mHitPoints - damage);

        player.mAllowedToMove = false;
        player.mPeriodicDamage = Math.round(procent2 * getPeriodicDamage());
        player.mPeriod = 1;
    }

    public void acceptAbilityFrom(final Player player) {
        player.applyAbilityTo(this);
    }

    protected void applyAbilityTo(final Pyromancer player) {
        doKnightAbility(player, P1_PYRO, P2_PYRO);
    }

    protected void applyAbilityTo(final Knight player) {
        doKnightAbility(player, P1_KNIG, P2_KNIG);
    }

    protected void applyAbilityTo(final Wizard player) {
        doKnightAbility(player, P1_WIZZ, P2_WIZZ);
    }

    protected void applyAbilityTo(final Rogue player) {
        doKnightAbility(player, P1_ROGU, P2_ROGU);
    }

    protected float getDamage1() {
        float damage = DMG_BASE1 + DMG_LEVEL1 * mLevel;
        if (mLocation == 'L') { // Land Bonus
            damage = LAND_BONUS * damage;
        }

        return damage;
    }

    protected float getDamage2() {
        float damage = DMG_BASE2 + DMG_LEVEL2 * mLevel;
        if (mLocation == 'L') { // Land Bonus
            damage = LAND_BONUS * damage;
        }

        return damage;
    }

    protected float getPeriodicDamage() {
        float damage = PER_DMG_BASE + PER_DMG_LEVEL * mLevel;
        if (mLocation == 'L') { // Land Bonus
            damage = LAND_BONUS * damage;
        }

        return damage;
    }
}
